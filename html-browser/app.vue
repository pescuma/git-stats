<template>
	<form style="margin-bottom: 30px">
		<fieldset>
			...
			<b-row>
				<b-select2 label="Authors" colspan="4" multiple allow-clear empty-text="All" :options="authors" :format="formatAuthor" :selected.sync="selectedAuthors"></b-select2>
				...
				<b-select2 label="First month" colspan="2" allow-clear empty-text="All" :options="months" :selected.sync="firstMonth"></b-select2>
				<b-select2 label="Last month" colspan="2" allow-clear empty-text="All" :options="months" :selected.sync="lastMonth"></b-select2>
			</b-row>
		</fieldset>
	</form>
...
<script>
	...
	function createFilesTab(data) {
		...
		createFullGrid('files-table', table, {
			files: false,
			languages: false
		});
	}
		...
		for (i = 0; i < items.length; ++i) {
			...
			result.push({
				...
				lines: {
					...
					code: itemData.filter(COL_LINE_TYPE, CODE)
								  .sum(),
					comment: itemData.filter(COL_LINE_TYPE, COMMENT)
									 .sum(),
					empty: itemData.filter(COL_LINE_TYPE, EMPTY)
								   .sum()
				},
				files: itemData.getDistinct(COL_FILE)
							   .count(),
				languages: itemData.getDistinct(COL_LANGUAGE)
								   .count(),
				commits: {
					total: itemData.getDistinct(COL_COMMIT)
								   .count(),
					start: mr[0],
					end: mr[1]
				},
				authors: itemData.getDistinct(COL_AUTHOR)
								 .count()
			});
		}
	...
	function createDonuts(div, table) {
		...
		for (var i = 0; i < itens.length; ++i) {
			...
			$("#" + div)
					.append("<div class='col-md-3'><div id='" + id + "' style='width: 100%; height: 300px'></div></div>");
	...
	function createPerMonthGraphs(div, table) {
		for (var i = 0; i < table.length; ++i) {
			...
			$("#" + div)
					.append("<div class='col-md-12'>" + "<div id='" + id + "' style='width: 100%; height: 200px'></div>" + "</div>");
	...
	function createPerMonthGraph(div, data, title) {
		...
		$.jqplot(div, [code, comments, empty], {
			...
			series: [{label: 'Code'}, {label: 'Comments'}, {label: 'Empty lines'}],
			legend: {
				show: true,
